...
Using {\tt Halotools} to forward model the observations, we test an empirical model in which present-day galaxy size is 
...
In closely related work, efforts to constrain correlations of galaxy size with halo properties at fixed stellar mass have been carried out using both weak lensing \citep[e.g.,][]{sonnenfeld_leauthaud18} and galaxy dynamics \citep[e.g.,][]{desmond_wechsler15,desmond_wechsler17}. Our results complement these previous studies in that forward-modeling galaxy observables directly into simulations provides a rigorous means by which one can test hypotheses of correlations implemented at fixed halo properties. In our fiducial model with $\mean{\rhalf}{\rmpeak}=0.01\rmpeak,$ for Milky Way mass centrals there exists a significant positive correlation between present day mass and galaxy size at fixed stellar mass; these alternative methods are a natural testbed for future tests of the basic conclusions drawn here based on two-point clustering. 
