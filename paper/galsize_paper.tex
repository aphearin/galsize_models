...
{\color{blue} AVK comment: The choice of nomenclature of $\rvir-$based model and $\mstar-$based model is not clear at all. APH: The big picture being compared/tested here are ``pre-infall" vs. ``post-infall" processes, respectively. Those don't seem like great names either though.}
...
The green curves in Figure~\ref{fig:mstarmodelclustering} test the clustering predictions of the $\mstar-$based model described in \S\ref{subsubsec:mstaronlymodel}. In this model, $\rhalf$ is a log-normal distribution centered at $\median{\rhalf}{\mstar}.$ By construction, this model predicts no dependence at all of galaxy clustering upon $\rhalf$ at fixed $\mstar,$ simply because the scatter of $\rhalf$ about $\median{\rhalf}{\mstar}$ is uncorrelated with any other property. This is in contrast to the $\rvir-$based model, which correlates the scatter with host halo mass and as well as halo assembly. Likewise, the $\mstar+$stripping model described in \S\ref{subsubsec:strippingmodel} also exhibits correlations between scatter in $\rhalf$ and halo properties: in this alternative model, satellites have smaller sizes than centrals of the same $\mstar$ due to post-infall stripping.

The treatment of stellar mass loss in this model naturally results in an enhancement of size differences between satellites and centrals. As shown by the purple curves in Figure \ref{fig:mstarmodelclustering}, this impacts $\rhalf-$dependent clustering in the expected manner. The ``small" galaxy population has a higher satellite fraction, and so in this model small galaxies cluster more strongly relative to large galaxies of the same stellar mass. However, the magnitude of the effect is evidently is not strong enough to produce clustering predictions that are consistent with SDSS. Evidently it is difficult to strip enough mass from satellites so that the $\rhalf-$dependent clustering is in agreement with observations. As we argue in \S\ref{sec:interpretation}, this supports the conclusion that the relative difference between central and satellite size is at least partially in place at the time of satellite infall. 
