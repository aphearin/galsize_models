...
\begin{abstract}
We explore size dependence of galaxy clustering and the implied connection between galaxy half-light radii, $\rhalf$, and their halo virial radii. 
We show that in the volume-limited SDSS galaxy samples smaller galaxies cluster much more strongly than larger galaxies at fixed stellar mass. 
The size dependence increases with decreasing scale and decreases with stellar mass of the sample. Using {\tt Halotools} to forward model the observations, 
we show that a simple empirical model, in which $\rhalf$ is proptortional to the halo $\rvir$ at the time when halo mass reached its maximum,
reproduces observed size-dependence of galaxy clustering in striking detail.
The success of the model implies that the size-dependence of galaxy clustering is mainly due to the fact that satellite galaxies have smaller sizes compared 
to central galaxies of the same stellar mass. Taken together, our findings indicate that  sizes of satellite galaxies are set  prior to satellite infall and that a 
remarkably simple linear size--virial radius relation emerges from the complex physics regulating galaxy size. The $\rvir$-based size model that reproduces
 the observed size dependence of clustering makes a prediction that future observations of size dependence of galaxy-galaxy lensing for stellar mass 
 selected galaxy samples will see stronger signal for smaller galaxies. 
 %Left over this sentence. Not sure we need to talk about this model in the abstract: 
 % At the same time, model in which size is based on stellar mass does not reproduce the clustering trend, even after taking into account stellar mass loss by satellites. 
...
on $\rhalf.$ To do so, we determine the value $\median{\rhalf}{\mstar}$ by computing a sliding median of $\rhalf$ for galaxies sorted by $\mstar,$ calculated using a window of width $N_{\rm gal}=1000.$ \textcolor{magenta}{AK: is window symmetric? What is done for the most massive and least massive galaxies? } Each galaxy is categorized as either ``large" or ``small" according to whether it is above or below the median value appropriate for its stellar mass. We note that this is analogous to the common convention for studying the properties of ``red" vs. ``blue" galaxies, in which the two subsamples are divided by a $\mstar-$ or luminosity-dependent green valley cut \citep[e.g.,][]{vdB_etal08,zehavi_etal11}, only here the $\rhalf$ distribution is uni-modal, not bi-modal.
...
\textcolor{magenta}{How is mass loss is handled and how stellar mass assigned for orphans? This is not described in the appendix or here.}.
...
As noted in \S\ref{sec:intro}, \citet{kravtsov13} showed that galaxy sizes are on average proportional to the virial radius of their host halo. Motivated by these results, we  explore a model in which $\rhalf$ scales linearly with halo virial radius:\footnote{We note that the normalization factor in \citet{kravtsov13} is 0.015, whereas our normalization is 0.01. The difference derives from two factors: {\em i.} the choice of halo radius definition: \citet{bryan_norman98} in the present work, compared to the $200c$ definition used in \citet{kravtsov13}; {\em ii.} the latter study used deprojected (larger) sizes for spheroidal galaxies, whereas we make no such correction here. See \S\ref{sec:future} for further discussion.}
\beq
...
\caption{
{\bf \textcolor{magenta}{AK: it may be difficult for people (especially color-blind) to tell the difference between color of points of small and large galaxies.} Distinct $\rhalf-$dependence of clustering for color-selected galaxy samples.}
...
\ben
\item Small galaxies cluster more strongly than large galaxies of the same stellar mass. Differences between the clustering of small and large galaxies increase on small scales $R\lesssim1\mpc,$ and decrease with stellar mass (see Figure \ref{fig:rvir_only_clustering_absolute}). 
\item The most important factor influencing this signal is the relative size of central and satellite galaxies. The magnitude, scale-dependence, and $\mstar-$dependence of $\rhalf-$dependent clustering provides strong evidence that satellite galaxies are smaller than central galaxies of the same halo mass.
\item A simple empirical model in which $\rhalf$ is proptortional to the halo $\rvir$ at the time when halo mass reached its maximum peak halo mass reproduces size-dependence of galaxy clustering of the SDSS galaxies in striking detail (see Figure \ref{fig:clustering_ratio_upshot}). 
\item Models in which $\rhalf$ is regulated by $\mstar,$ rather than $\mhalo,$ are grossly discrepant with the observed clustering signal, even when accounting for satellite mass stripping (see Figure \ref{fig:mstarmodelclustering}).
...
\item The $\rvir$-based size model that reproduces the observed size dependence of clustering makes a prediction that future observations of size dependence of galaxy-galaxy
lensing for stellar mass selected galaxy samples will see stronger signal for smaller galaxies. 
\een
...
\caption{
{\bf Impact of the orphan selection function.} \textcolor{magenta}{AK: I think the 4-panel plot should be a separate figure. Otherwise it's too small.}. The {\em top panel} is identical to Figure \ref{fig:baseline_sham_clustering}, but includes additional curves for the case of $M_{\rm host}-$dependent orphan selection (orange curves), and no orphans at all (red curves). As a function of stellar mass, the {\em bottom panel} shows the satellite fraction, i.e., the fraction of objects that are not central galaxies.
}
