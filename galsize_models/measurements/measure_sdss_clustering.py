...
umachine_sdss_fname = (
    "/Users/aphearin/Dropbox/UniverseMachine/data/sdss/dr10_sm_sfr_complete3.dat"
)
...
def _copy_auto_regions_data_file(dirname_to_sample_ascii):
    ...
    if os.path.isfile(auto_regions_fname1):
        ...
        if os.path.isfile(auto_regions_fname2):
            ...
            raise IOError(
                "The ``auto_regions.txt`` file could not be found in your current directory,\n"
                "nor in the Dropbox location {1}".format(auto_regions_fname2)
            )


def write_umachine_ascii(
    gals, fname, orig_keys=list(_orig_umachine_sdss.keys()), overwrite=False
):
    output_table = Table()
...
def get_wp_measurements(wp_sample_fname, sm_low, sm_high, pi_max, num_randoms=int(2e5)):
    ...
    code_basename = (
        "/Users/aphearin/work/UniverseMachine/code/UniverseMachine/correl/correl"
    )
...
def measure_wp(
    ...
    rp, wp, wperr = result[:, 0], result[:, 1], result[:, 2]
    one_plus_wp_interp = np.exp(
        np.interp(np.log(rp_bins), np.log(rp), np.log(1.0 + wp))
    )
    wperr_interp = np.exp(np.interp(np.log(rp_bins), np.log(rp), np.log(wperr)))
    return rp_bins, one_plus_wp_interp - 1.0, wperr_interp


def below_median_size_mask(log10sm, rhalf):
    median_log10_size, log10sm_bins, __ = binned_statistic(
        log10sm, np.log10(rhalf), statistic="median"
    )
    logsm_mids = 0.5 * (log10sm_bins[:-1] + log10sm_bins[1:])
    return rhalf < 10 ** np.interp(log10sm, logsm_mids, median_log10_size)


def save_wp_measurement(
    sample, output_dirname, output_basename, sm_low, sm_high, pi_max=20.0
):
    """ """
    assert (
        len(sample) > 900
    ), "Sample size = {0}.\nCan only measure clustering for at least 900 galaxies".format(
        len(sample)
    )
    ...
    rp, wp, wperr = measure_wp(sample, sm_low, sm_high, output_dirname)

    rp_fname = os.path.join(output_dirname, output_basename + "_rp")
    wp_fname = os.path.join(output_dirname, output_basename + "_wp")
    wperr_fname = os.path.join(output_dirname, output_basename + "_wperr")
    np.save(wp_fname, wp)
    np.save(wperr_fname, wperr)
    np.save(rp_fname, rp)
