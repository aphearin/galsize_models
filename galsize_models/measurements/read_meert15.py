...
def load_meert15(datadir=default_datadir, phot_type=4):
    ...
    t['Magr_tot'] = (t['magr_tot'] - 5.0*np.log10(t['lum_dist']/1e-5) -
            t['extmr'] + 1.3*t['z'] - t['kcorr'])
    ...
    t['r50_magr_kpc'] = t['r50_magr_arcsec'] * np.pi * t['ang_dist'] * 1000.0/(180.*3600.)
    t['r50_magg_kpc'] = t['r50_magg_arcsec'] * np.pi * t['ang_dist'] * 1000.0/(180.*3600.)
    ...
    t['morph_type_T'] = -4.6*t['probaEll'] - 2.4*t['probaS0'] + 2.5*t['probaSab'] + 6.1*t['probaScd']
    ...
    t['Magr_bulge'] = (t['magr_bulge'] - 5.0*np.log10(t['lum_dist']/1e-5) -
            t['extmr'] + 1.3*t['z'] - t['kcorr'])
    t['Magr_disk'] = (t['magr_disk'] - 5.0*np.log10(t['lum_dist']/1e-5) -
            t['extmr'] + 1.3*t['z'] - t['kcorr'])
    t['Lumr_bulge'] = 10**0.4*(4.67-t['Magr_bulge'])
    t['Lumr_disk'] = 10**0.4*(4.67-t['Magr_disk'])

    bulge_term = 10.**(-t['Magr_bulge']/2.5)
    disk_term = 10.**(-t['Magr_disk']/2.5)
    ...
    mask = ((t['z'] > 0.001) & (t['z'] < 0.2) &
            (t['magr_tot'] > 14.) & (t['magr_tot'] < 17.77) &
            (t['r50_magr_arcsec'] > 0.75) & (t['extmr'] > 0.) &
            (t['magg_tot'] > 0.) & (t['extmg'] > 0.) &
            (t['gr_kcorr'] > -0.5) & (t['gr_kcorr'] < 2.2))
