...
def sdss_size_vs_stellar_mass(
    logsm, size, redshift, logsm_bins, statistic="mean", zmin=0.02
):
    """
    Parameters
    ----------
    ...
    completeness_table = np.loadtxt(
        os.path.join(completeness_table_dirname, "completeness.dat")
    )
...
def mock_ssfr_sequence_one_points(
    logsm_bins, logsm, r50_kpc, mask_sf, mask_gv, mask_q, statistic="mean"
):
...
def tabulate_sdss_size_vs_stellar_mass(output_dirname=os.path.abspath(".")):
    ...
    mask_gv = (
        (full_sdss["ssfr"] <= -10.75)
        & (full_sdss["ssfr"] > -11.25)
        & meert15_measurement_mask
    )
