...
__all__ = (
    "wp_size_ratios_mock",
    "single_component_ratios",
    "single_component_clustering",
)
...
def single_component_ratios(
    ...
    for logsm_cut in (9.75, 10.25, 10.75, 11.25):
        ...
        if len(sample) > num_gals_max:
            downsampling_mask = np.random.choice(
                np.arange(len(sample)), num_gals_max, replace=False
            )
...
def single_component_clustering(
    mock,
    rp_bins=np.logspace(-1, 1.25, 20),
    pi_max=20.0,
    period=250.0,
    ...
    for logsm_cut in (9.75, 10.25, 10.75, 11, 11.25):
        sample = mock[mock["sm"] > logsm_cut]
        if len(sample) > num_gals_max:
            downsampling_mask = np.random.choice(
                np.arange(len(sample)), num_gals_max, replace=False
            )
        ...
        result_collector.append(tuple((wp_small, wp_all, wp_large)))
